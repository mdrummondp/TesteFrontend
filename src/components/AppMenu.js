...
const MenuItemLink = ({ item, className }) => {
    return (
        <Link to={item.path} className={classNames('side-nav-link-ref', 'side-sub-nav-link', className)} style={{fontSize:25, color:'black'}}>
            
            {item.badge && <span>{item.badge.text}</span>}
            <div style={{display:'inline-block', textAlign:'center', color:'black', background: 'linear-gradient(180deg, rgba(255,255,255,1) 0%, rgba(144,144,144,1) 35%, rgba(255,255,255,1) 105%)', borderRadius:'15px 15px', width:'105%', height:'90%', marginLeft:'5%'}}>
                <span> {item.name}</span>
            </div>
        </Link>
    );
